		...
		Pass
		{
			...
			CGPROGRAM
				...
				float4 frag(PS_INPUT i) : SV_Target
				{
					...
					//float3 vNormalTs = UnpackScaleNormal( tex2D(_GrabTexture, i.vTexCoord0.xy ), 1 );
					float3 vNormalTs = UnpackScaleNormal( tex2Dproj(_GrabTexture, i.vGrabPos), 1 );
					vNormalTs = normalize(mul((float3x3)UNITY_MATRIX_V, vNormalTs));
