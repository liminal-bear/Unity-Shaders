...
public class LitDitherFadeEditor : ShaderGUI
{
    ...
    void DrawUI()
    {
		...
		MaterialProperty fracOpacity = FindProperty("_FracOpacity", properties);
		GUIContent fracOpacityLabel = new GUIContent("Fractional Opacity (quick dither)", "Percent chance a pixel will be transparent");
		editor.ShaderProperty(fracOpacity, fracOpacityLabel);

		MaterialProperty fracDistOpacity = FindProperty("_DistFracOpacity", properties);
		GUIContent fracDistOpacityLabel = new GUIContent("Fractional Distance Opacity (quick dither by distance)", "use distance to influence transparency");
		editor.ShaderProperty(fracDistOpacity, fracDistOpacityLabel);

        //MaterialProperty quantPos = FindProperty("_QuantPos", properties);
        //GUIContent quantPosLabel = new GUIContent("Quantize vertices", "Controls the world space rounding of vertices (like ps1)");
        //editor.ShaderProperty(quantPos, quantPosLabel);


        MaterialProperty quantize = FindProperty("_Quantize", properties);
        GUIContent quantizeLabel = new GUIContent("Quantize vertices", "Controls the world space rounding of vertices (like ps1)");
        editor.ShaderProperty(quantize, quantizeLabel);
