Shader "LitDitherFade"
{
	Properties
	{
		...
		_TintColor("Tint Color", Color) = (1,1,1,1)
			...
			_FracOpacity("Fractional Opacity (quick dither)", Range(0,1)) = 1
			_DistFracOpacity("Fractional Opacity (distanceBased)", Range(0,10)) = 3.19

			_Quantize("Quantize", Range(1, 500)) = 456
		...
		//#include "UnityShadowLibrary.cginc"
			...
			float _FracOpacity, _DistFracOpacity;
			...
			SubShader
			{
				...
				Pass
				{
					...
					float4 frag(vertexOutput input) : COLOR
					{
						...
						//float fracOpacity = random(input.pos.z) > _FracOpacity ? 0 : 1;
						float fracOpacity = random(input.posWorld.x + input.posWorld.y + input.posWorld.z) >= _FracOpacity ? 0 : 1;
						//float fracOpacity = random(Quantize(input.posWorld.x + input.posWorld.y + input.posWorld.z, 100)) >= _FracOpacity ? 0 : 1;
						//float fracOpacity = random(Quantize(input.posWorld.x + input.posWorld.y + input.posWorld.z, 100)) >= _FracOpacity ? 0 : 1;
						//float fracOpacity = random(Quantize(input.posWorld.x + input.posWorld.y + input.posWorld.z, _Quantize)) >= _FracOpacity ? 0 : 1;
						//float fracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.posWorld.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) >= _FracOpacity ? 0 : 1;

						//float distFracOpacity = random(input.posWorld.x + input.posWorld.y + input.posWorld.z) <= input.pos.w * _DistFracOpacity ? 0 : 1;
						//float distFracOpacity = random(input.pos.x + input.pos.y + input.pos.z) <= input.pos.w * _DistFracOpacity ? 0 : 1;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) + input.pos.z* 2 <= input.pos.w * _DistFracOpacity ? 0 : 1;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.posWorld.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) + input.pos.z * 2 <= input.pos.w * _DistFracOpacity ? 0 : 1;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) / 2 <= Quantize(input.pos.w, _Quantize) * _DistFracOpacity ? 0 : 1;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) <=  _DistFracOpacity ? 1 : 0;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) <=  _DistFracOpacity ? 1 : 0;
						//float depthComponent = input.pos.w * input.pos.w * input.pos.w * input.pos.w * _DistFracOpacity;
						//float depthComponent = input.pos.w * input.pos.w * input.pos.w * input.pos.w * _DistFracOpacity;
						float depthComponent = input.pos.w * input.pos.w * input.pos.w * input.pos.w * _DistFracOpacity;
						depthComponent = Quantize(depthComponent, _Quantize);
						//distFracOpacity = distFracOpacity == 100 ? 1 : distFracOpacity;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) + input.pos.z >= depthComponent ? 1 : 0;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) + Quantize(input.pos.z, _Quantize) >= depthComponent ? 1 : 0;
						float distFracOpacity = random(Quantize(input.posWorld.x-98, _Quantize) * Quantize(4*(input.posWorld.y), _Quantize) * Quantize(input.posWorld.z, _Quantize)) + Quantize(input.pos.z, _Quantize) >= depthComponent ? 1 : 0;
						//float distFracOpacity = random(Quantize(input.posWorld.x, _Quantize) + Quantize(input.pos.y, _Quantize) + Quantize(input.posWorld.z, _Quantize)) + input.pos.z;


						return float4(col.rgb, col.a * _Opacity * fracOpacity * distFracOpacity);
						//return float4(col.rgb, col.a * _Opacity * fracOpacity * distFracOpacity);
						//return float4(distFracOpacity, distFracOpacity, distFracOpacity,1);
						//return float4(depthComponent, depthComponent, depthComponent,1);
			...
			Pass
			{
				...
				#pragma multi_compile_fwdadd_fullshadows
					...
					float4 frag(vertexOutput input) :SV_Target
					{
						...
						//return float4(col.rgb, 1 * _Opacity * fracOpacity);
				...
				//	shadowVertexOutput vertSShadow(shadowVertexInput input)
				//	{
				...
				//		output.pos.z = lerp(output.pos.z,min(output.pos.z, output.pos.w * UNITY_NEAR_CLIP_VALUE), unity_LightShadowBias.y);
				//		return output;
				//	}
				//	//fixed4 fragSShadow(float4 pos : SV_POSITION) : SV_Target{return 0;}
				//	fixed4 fragSShadow(shadowVertexOutput input) : COLOR
				//	{
				...
				//		//clip(fracOpacity * distFracOpacity);

				//		//if (fracOpacity * distFracOpacity == 0)
				//		//{
				//		//	clip(-1);
				//		//}

				//		return float4(0,0,0, fracOpacity * distFracOpacity);
				//	}
				//	ENDCG
				//}
				//Pass
