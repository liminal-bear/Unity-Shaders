Shader "LitTransformable"
{
	Properties
	{
		...
		//#include "UnityShadowLibrary.cginc"
			...
			uniform float4 _AxisAdjust;
			uniform int _QuantPos;
			...
			vertexOutput vert(vertexInput input)
			{
				...
				if (_QuantPos < 500)
				{
					...
					worldVertex.y = Quantize(worldVertex.y, _QuantPos);
					worldVertex.z = Quantize(worldVertex.z, _QuantPos);
					worldVertex.w = Quantize(worldVertex.w, _QuantPos);

					//add a little bit of the original shape to prevent double planing
			...
			ENDCG
			CustomEditor "LitTransformableEditor"
			SubShader
			{
				Tags
				{
				...
				Pass
				{
					...
					float4 vertSShadow(float4 vertex : POSITION) : SV_POSITION{
						...
						if (_QuantPos < 500)
						{
							...
							//add a little bit of the original shape to prevent double planing
							worldVertex.x += vertex.x * 0.001;
							worldVertex.y += vertex.y * 0.001;
							worldVertex.z += vertex.z * 0.001;
							worldVertex.w += vertex.w * 0.001;

							vertex = mul(unity_WorldToObject, worldVertex);
						}
